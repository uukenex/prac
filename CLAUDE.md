...
## 세션 시작 시 최우선 확인
- **리포지토리 루트에 `HANDOFF.md`가 있으면, 다른 어떤 작업보다 먼저 그 내용부터
  (그대로 복붙하지 말고 다시 정리해서) 사용자에게 알려줄 것.** 특히 "다른컴퓨터에서
  작업한 것 확인해줘" 같은 요청을 받았을 때 최우선 순위. 관련 작업이 실제로 마무리되면
  `HANDOFF.md`를 삭제하거나 최신 상태로 갱신할 것.
...
## DB 스크립트 실행 (sqlplus)
- **DB 접속 정보(계정/비번/host:port/SERVICE_NAME) 기억 정책**(2026-09-04 최초 지시,
  2026-09-12 완화): 사용자 본인 컴퓨터(로컬 세션)에서 작업할 때는 Claude 메모리(리포지토리
  밖, `~/.claude/projects/.../memory/`)에 접속정보를 기억해두고 재사용해도 된다. 단
  **어떤 경우에도 이 정보를 리포지토리 파일이나 커밋에 남기지 말 것** — `.sql` 마이그레이션
  파일, CLAUDE.md 등 git으로 추적되는 파일에는 절대 실제 계정/비번을 적지 않는다(메모리
  파일만 예외). 로컬이 아닌 환경(다른 사용자, 원격/클라우드 세션 등)에서는 여전히 매번
  물어볼 것.
