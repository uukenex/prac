...
## [2026-09-10] S5(탑) PP 연동 -- 낚시 성공 시 탑 PP 보너스

"낚시로 인한 PP 획득은 S5 유저만 받도록, 아니면 멘트도 안 나오게, 9/11부터 적용" 요청.

- `/낚시`는 이미 하루 1회로 컨트롤러에서 막혀 있어서(`TBOT_S4_FISHING_LOG` PK가
  (USER_NAME, FISHING_DATE)) 별도의 "하루 1회" 제한 로직 없이도 자연히 일일 보너스가 된다.
- `BotS4ServiceImpl.fishing()`이 매 낚시 성공 후 `BotS5Service.grantFishingBonus(userName,
  fishGrade)`를 호출 -- PP 지급 자격/금액 산정은 전부 S5 쪽에 위임(S4는 잡은 물고기 등급
  1~8만 넘겨줌).
- `grantFishingBonus()`는 (a) 오늘이 2026-09-11 이전이면, (b) 이 유저가 S5(탑) 진행기록
  자체가 없으면(=`TBOT_S5_USER_PROGRESS` 행 없음, 탑을 한 번도 안 해본 유저) `null`을
  반환 -- S4 쪽은 `null`이면 보상은 물론 관련 문구 자체를 통째로 생략한다("S5 유저가
  아니면 멘트도 안 나오게" 요청 그대로).
- 지급액 설계(라이브 데이터 확인 후 결정): S5 유저 23명(평균 25.6층/중앙값 23층), 그중
  21명이 S4도 같이 함(대부분 겹침). 실제 낚시 등급 분포는 ★1이 59%로 압도적, ★2~★3이
  그다음, ★6까지는 종종 나오고 ★7은 극히 드묾(★8은 아직 아무도 없음, rod5 미해금).
  이 유저의 "현재 최고 도달층 기준 몬스터 1마리 처치 PP"(`PP_PER_KILL_VALUE ×
  floorPpMultiplier`, S5의 일반 처치 보상과 동일한 산식)에 잡은 물고기 등급별 배율
  (`FISH_GRADE_PP_MULT = {1x, 1.5x, 2x, 3x, 4x, 6x, 8x, 12x}`, ★1~★8)을 곱해서 지급 --
  1층 유저의 ★1 낚시는 1PP 남짓, 58층 유저의 ★6 낚시는 1000PP 넘는 식으로 항상 그 유저의
  경제 규모에 비례한다. 코드/상수는 `BotS5ServiceImpl.grantFishingBonus()`,
  `FISHING_PP_START_DATE`, `FISH_GRADE_PP_MULT` 참고 -- DB 마이그레이션 없음(기존 PP 지급
  경로 `addPp()`를 그대로 재사용), 정적 리소스만 변경, 재배포 필요.
