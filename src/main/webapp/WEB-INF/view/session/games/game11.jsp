...
<BODY onload="init()">
	...
	function init()
    {
		...
		var v_boss_tot_energy 	  = 40;
		var v_boss_tot_energy_max = 40;
    	...
    	$(function(){
    		...
    		sub_interval = setInterval(function(){
        		...
        		//보스 생성
        		if(v_score_hit >= 25 && !flag_boss_attack ){
    	...
    	function getMyPosY(){
    		...
    		var top = $char.position().top;
    		//var left = $char.position().left;
    	...
    	function f_enemy_create(sp_count,e_speed,x,y){
    		...
    		//y = 생성위치 
    		//yPos = 내위치
    		var y_speed= Math.abs(yPos-y)/150;
    		//생성위치에 따른 방향처리
    		if(y > yPos) {
    			//생성위치가 더 아래쪽인경우 적이 위쪽방향으로 이동해야한다 
    			y_speed = -y_speed;
    		}
